...
- **模型后端** — 以下任选其一：
  - [Ollama](https://ollama.com/)（推荐，本地开发无需 API key）
  - Anthropic API key
...
# 单次提问
cargo run -- --provider ollama -m "qwen3.5:9b" -q "列出当前目录的文件"

# 交互式 REPL
cargo run -- --provider ollama -m "qwen3.5:9b"
```

支持工具调用的模型都可以使用，更大的模型工具调用效果更好：

```bash
cargo run -- --provider ollama -m "qwen3.5:27b" -q "读一下 Cargo.toml，总结这个 workspace"
```
...
Options:
  -m, --model <MODEL>          模型名称（默认根据 provider 自动选择）
  -s, --system <SYSTEM>        系统提示词
  -p, --permission <MODE>      权限模式：auto, interactive, deny
  -q, --query <QUERY>          单次提问（非交互），省略则进入 REPL
      --provider <PROVIDER>    后端：anthropic, ollama, openai, stub
      --ollama-url <URL>       Ollama 服务地址（默认 http://localhost:11434）
      --max-turns <N>          单次对话最大轮数（默认 100）
```
...
| Provider | 后端 | 激活方式 |
|----------|------|----------|
| `ollama` | Ollama（本地） | `--provider ollama` 或未设置 `ANTHROPIC_API_KEY` 时自动选择 |
| `anthropic` | Anthropic API | 设置 `ANTHROPIC_API_KEY` 环境变量 |
| `openai` | 任意 OpenAI 兼容 API | `--provider openai` + `OPENAI_BASE_URL` |
| `stub` | 无模型调用（测试用） | `--provider stub` |
...
屏蔽不同模型后端的差异，统一流式输出、重试和错误恢复逻辑。支持 Anthropic API、OpenAI 兼容 API（Ollama、vLLM 等）。
</details>
...
```text
rust-clw/
├── Cargo.toml                          # Workspace 根 — 声明所有 crate 和公共依赖
├── Cargo.lock
├── crates/
│   ├── claude-cli/                     # 可执行入口
│   │   └── src/main.rs                 #   CLI 参数、REPL 循环、provider/tool 组装
│   │
│   ├── agent-core/                     # 消息模型、会话状态、Agent 主循环
│   │   └── src/
│   │       ├── lib.rs
│   │       ├── message.rs              #   Role, ContentBlock, Message
│   │       ├── session.rs              #   SessionConfig, SessionState
│   │       ├── query.rs                #   递归 Agent 循环（核心心跳）
│   │       └── error.rs                #   AgentError 枚举
│   │
│   ├── agent-tools/                    # 工具 trait、注册表、编排器
│   │   └── src/
│   │       ├── lib.rs
│   │       ├── tool.rs                 #   Tool trait, ToolOutput
│   │       ├── registry.rs             #   ToolRegistry — 名称 → 实现 查找
│   │       ├── orchestrator.rs         #   ToolOrchestrator — 批量调度与权限
│   │       └── context.rs              #   ToolContext — 注入给工具的最小依赖
│   │
│   ├── agent-provider/                 # 统一模型 provider 抽象
│   │   └── src/
│   │       ├── lib.rs
...
│   │       ├── anthropic.rs            #   Anthropic API 实现（SSE 流式解析）
│   │       └── openai_compat.rs        #   OpenAI 兼容实现（Ollama、vLLM 等）
│   │
│   ├── agent-permissions/              # 授权层
│   │   └── src/
│   │       ├── lib.rs
│   │       ├── types.rs                #   PermissionMode, ResourceKind, PermissionDecision
│   │       ├── policy.rs               #   PermissionPolicy trait
│   │       └── rules.rs                #   RuleBasedPolicy — glob/前缀规则匹配
│   │
│   ├── agent-tasks/                    # 后台任务生命周期
│   │   └── src/
│   │       ├── lib.rs
│   │       ├── task.rs                 #   Task trait, TaskState, TaskInfo, TaskNotification
│   │       └── manager.rs              #   TaskManager — 注册、追踪、通知、取消
│   │
│   ├── agent-compact/                  # 上下文压缩与 token 预算
│   │   └── src/
│   │       ├── lib.rs
│   │       ├── budget.rs               #   TokenBudget — 窗口大小、阈值
│   │       └── strategy.rs             #   CompactStrategy trait, TruncateStrategy
│   │
│   ├── agent-mcp/                      # MCP 集成（Phase 3 占位）
│   │   └── src/
│   │       └── lib.rs                  #   McpServerConfig 类型占位
│   │
│   └── tools-builtin/                  # 内置工具实现
│       └── src/
│           ├── lib.rs                  #   register_builtin_tools()
│           ├── bash.rs                 #   BashTool — Shell 命令执行
│           ├── file_read.rs            #   FileReadTool — 按行范围读取文件
│           └── file_write.rs           #   FileWriteTool — 写入文件（自动创建目录）
│
├── docs/
│   ├── assets/
│   │   ├── overview.svg                # 项目总览图
